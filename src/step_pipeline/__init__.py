...
You can also create compound Steps which contain substeps that are skipped or forced as a group.
...
class _Step(ABC):
    ...
    def command(self, command):
        """Adds a command"""
        #if self._substeps:
        #    raise ValueError(f"Cannot add command '{command}' to step {self.short_name} because it contains substeps. "
        #                     f"A step can contain either commands or substeps but not both.")
    ...
    def input(self,
        ...
        destination_dir = os.path.join(destination_root_dir, subdir, input_spec["source_dir"])
    ...
    def output(self, local_path, destination_dir=None, destination_filename=None, name=None, delocalization_strategy=DelocalizationStrategy.COPY):
        if not self._output_dir and not destination_dir:
            raise ValueError("No destination dir specified")
        ...
        filename = destination_filename or os.path.basename(local_path)
        output_spec = {
    ...
    #    """Factory method to create a new step that's a substep of this method
    #    """
    #    # TODO crawl the containment heirarchy to make sure there are no loops?
    #    if self._commands:
    #        raise ValueError(f"Cannot add substep to step {self.short_name} because it contains commands. "
    #                         f"A step can contain either substeps or commands but not both.")
